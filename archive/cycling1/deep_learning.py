...
class GroundTruthGPX:
	...
	def __init__(self, filename, gps, load_cache=False):
	    ...
	    time_vector = []
	    indexes = []
	    for position in self.dis:  ## For each ground truth position find closest gps
	        distances = gps[:, 1:3] - position
	        magnitudes = np.linalg.norm(distances, ord = 2, axis = 1)
	        indexes.append(np.argmin(magnitudes)) ## Store minimum gps index
	        time_vector.append([gps[indexes[-1], 0]])

	    self.dis = np.concatenate((np.asmatrix(time_vector), self.dis), axis=1)  ## Add time column
	    self.dis[-1] = gps[-1, 0:3]  
	    indWe're sick of Miss C.H and her male friends trying to target our store so let's get them identified and cexes[-1] = len(gps)
...
training_files = cycling_files[:-2]
testing_files  = cycling_files[-2:]
